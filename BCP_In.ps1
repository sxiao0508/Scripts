...
Function BCP_In { 
    param(
        ...
        [String] $Path       #Source data folder
    )
    #Log file
    $Log = $Path+"\Status.txt"
    #data folder
    ...
    if ($exists -ne $True) {
        "$date" + " : program existed as folder doesn't exist" | Out-File $Log -Append
        Exit
    }
    ...
    Get-ChildItem $Folder | ForEach-Object {
        ...
        #Foreach of databases
        Get-Content "$Path\Databases.txt" | ForEach-Object {

            $Database = $_;
            $date = Get-Date    
            "$date" + " : Starting BCP: "+ "$Database.$Table" | Out-File $Log -Append       

            sqlcmd -S $Sqlserver -Q "TRUNCATE TABLE $Table;" -E -d $Database
            bcp.exe $Table in $File -T -S $Sqlserver -d $Database -b 50000 -n

            $date2 = Get-Date;
            "$date2" + " : Ending BCP: "+ "$Database.$Table " + ($date2 - $date).TotalSeconds  | Out-File $Log -Append
        }
    }
}
...
#BCP_In "localhost" "C:\src\sqlproject\Bcp"
